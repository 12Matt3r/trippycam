...
// Feedback Displace Shader
const feedbackDisplaceShaderGLSL = `
...
const displacementMapShaderGLSL = `
    precision highp float; varying vec2 v_texCoord;
    uniform sampler2D u_webcamTexture; uniform sampler2D u_previousFrameTexture;
    uniform float u_time; uniform float u_brightness; uniform float u_contrast; uniform float u_saturation; uniform float u_intensity;
    ...
    void main() {
        ...
        vec4 prevFrameColor = texture2D(u_previousFrameTexture, screenUV);
        float prevFrameLuminance = getLuminance(prevFrameColor.rgb);
        float actualDisplacementStrength = u_displacementStrength * (0.5 + u_intensity * 0.5);
        vec2 displacementOffset = vec2(prevFrameLuminance - 0.5) * actualDisplacementStrength;
        vec2 webcamSampleUV = vec2(1.0 - screenUV.x, screenUV.y);
        vec2 displacedWebcamSampleUV = webcamSampleUV + displacementOffset;
        vec4 finalColor;
        if (displacedWebcamSampleUV.x < 0.0 || displacedWebcamSampleUV.x > 1.0 || displacedWebcamSampleUV.y < 0.0 || displacedWebcamSampleUV.y > 1.0) {
            finalColor = vec4(0.0, 0.0, 0.0, 1.0);
        } else {
            finalColor = texture2D(u_webcamTexture, displacedWebcamSampleUV);
        }
        finalColor.rgb = adjustSaturation(adjustBrightnessContrast(finalColor.rgb, u_brightness, u_contrast), u_saturation);
        gl_FragColor = finalColor;
}`;
...
void main() {
    ...
    gl_FragColor = finalColor;
        ...
        let controlValues = { 
            ...
            fisheyeStrength: 0.3,
            glitchStrength: 0.1,
            displacementMapStrength: 0.02, // Feedback Displace
        };
        ...
        let shaderSources = {
            ...
            crt: crtShader,
            mirror: mirrorShaderGLSL,
            wavewarp: waveWarpShaderGLSL,
            kaleidoscope: kaleidoscopeShaderGLSL,
            noiseGlitch: noiseGlitchShaderGLSL,
            fisheye: fisheyeShaderGLSL,
            feedbackDisplace: feedbackDisplaceShaderGLSL
        };

        const shaderControlRelevance = {
            'datamosh': ['intensity', 'displacement', 'feedback'],
            'pixelsort': ['intensity', 'displacement', 'feedback', 'threshold'],
            'feedback': ['intensity', 'displacement', 'feedback', 'threshold'],
            'colorshift': ['intensity', 'displacement', 'feedback', 'threshold'],
            'crt': ['scanlineIntensity', 'scanlineDensity', 'curvatureAmount', 'phosphorOffset', 'vignetteStrength', 'vignetteSoftness'],
            'mirror': [],
            'wavewarp': ['waveDensity', 'waveSpeed', 'waveAmplitude'],
            'kaleidoscope': ['kaleidoSegments'],
            'fisheye': ['fisheyeStrength'],
            'noiseGlitch': ['glitchStrength'],
            'feedbackDisplace': ['displacementMapStrength', 'intensity']
        };
        ...
        async function setupAudio() {
            ...
            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                showError("Audio Reactivity Error: getUserMedia not supported.", false);
                return false;
            }
            try {
                ...
                requestAnimationFrame(processAudioFrame); // Start processing loop
                return true;
            } catch (err) {
                showError(`Audio Reactivity Error: ${err.name} - ${err.message}. Check mic permissions.`, false);
                isAudioSetup = false;
                return false;
            }
        }
        ...
        function processAudioFrame() {
            if (!isAudioSetup || !analyserNode || !audioDataArray) {
                if (audioReactiveEnabled) requestAnimationFrame(processAudioFrame); // Keep trying if enabled
                return;
            }
            ...
            for (let i = 0; i < audioDataArray.length; i++) {
                const normalizedValue = audioDataArray[i] / 255.0;
                sumOfSquares += normalizedValue * normalizedValue;
            }
            const rms = Math.sqrt(sumOfSquares / audioDataArray.length);
            const smoothingFactor = 0.1;
            smoothedVolume = rms * smoothingFactor + smoothedVolume * (1.0 - smoothingFactor);
            smoothedVolume = Math.min(1.0, Math.max(0.0, smoothedVolume));
            if (audioReactiveEnabled) {
        ...
        function setupMainShaderAttributesUniforms() {
            ...
            const uniformNames = [
                ...
                'u_waveDensity', 'u_waveSpeed', 'u_waveAmplitude',
                'u_kaleidoSegments', 'u_fisheyeStrength', 'u_glitchStrength',
                // 'u_fisheyeStrength' // Duplicate removed
                'u_displacement_map_strength', 'u_displacement_texture_feedback', 'u_resolution'
            ];
            
            /* @tweakable tolerance for missing uniforms in shaders */
            const optionalUniforms = [
                'u_threshold', 'u_scanlineIntensity', 'u_scanlineDensity', 'u_curvatureAmount',
        ...
        const controlConfig = { 
            ...
            waveDensity: { precision: 1, isInt: false },
            waveSpeed: { precision: 2, isInt: false },
            waveAmplitude: { precision: 3, isInt: false },
            kaleidoSegments: { precision: 0, isInt: true },
            glitchStrength: { precision: 2, isInt: false },
            fisheyeStrength: { precision: 2, isInt: false },
            displacementMapStrength: { precision: 3, isInt: false, label: 'Displacement Strength'} // Feedback Displace
        };
        ...
        function drawScene(time) {
            ...
            // Note: Some uniforms (e.g., u_threshold) are only used by specific shaders. Their corresponding UI controls won't affect visuals if a shader that doesn't use them is active.
            ...
            let currentIntensity = controlValues.intensity; // Base intensity from UI
            if (audioReactiveEnabled && isAudioSetup) {
                currentIntensity = Math.min(1.0, controlValues.intensity + smoothedVolume * 0.75);
            }
            safeUniform1f('u_intensity', currentIntensity);
            ...
            safeUniform1f('u_waveDensity', controlValues.waveDensity);
            safeUniform1f('u_waveSpeed', controlValues.waveSpeed);
            safeUniform1f('u_waveAmplitude', controlValues.waveAmplitude);
            safeUniform1f('u_kaleidoSegments', controlValues.kaleidoSegments);
            safeUniform1f('u_fisheyeStrength', controlValues.fisheyeStrength);
            safeUniform1f('u_glitchStrength', controlValues.glitchStrength);
            safeUniform1f('u_displacement_map_strength', controlValues.displacementMapStrength);

            // For feedbackDisplaceShader, u_displacement_texture_feedback should sample from the same texture as u_previousFrameTexture (TEXTURE1)
            const displacementTextureFeedbackLoc = shaderProgram['u_displacement_texture_feedbackLoc'];
            if (displacementTextureFeedbackLoc !== null && displacementTextureFeedbackLoc !== undefined) {
                 gl.uniform1i(displacementTextureFeedbackLoc, 1); // TEXTURE1 is u_previousFrameTexture
            }

            const resolutionLoc = shaderProgram['u_resolutionLoc'];
            if (resolutionLoc !== null && resolutionLoc !== undefined) {
                if (canvas && typeof canvas.width === 'number' && typeof canvas.height === 'number') {
                    gl.uniform2f(resolutionLoc, canvas.width, canvas.height);
                } else {
                    gl.uniform2f(resolutionLoc, 640.0, 480.0); // Fallback resolution
                }
            }
        ...
        function changeEffect(effectName) {
            ...
            // --- Start of Dynamic UI update logic ---
            const effectsTab = document.getElementById('effects-tab');
            if (effectsTab) {
                const allEffectControls = effectsTab.querySelectorAll('div.control-group[data-control-key]');
                const relevantKeys = shaderControlRelevance[effectName] || [];

                allEffectControls.forEach(controlDiv => {
                    const controlKey = controlDiv.dataset.controlKey;
                    if (relevantKeys.includes(controlKey)) {
                        controlDiv.style.display = ''; // Revert to default CSS (flex)
                    } else {
            ...
            const effectTitleElement = document.getElementById('currentEffectTitle');
            if (effectTitleElement) {
                effectTitleElement.textContent = effectName ? `${effectName} Controls` : 'Effect Controls';
            }
            // --- End of Dynamic UI update logic ---
        ...
        async function main() {
            ...
            // Changing effects will load different shaders. Some UI controls in the 'Effects' tab (e.g., 'Threshold') might not be relevant for all selected effects.
            ...
            const audioReactiveCheckbox = document.getElementById('audioReactiveCheckbox');
            if (audioReactiveCheckbox) {
                audioReactiveCheckbox.checked = audioReactiveEnabled;
                audioReactiveCheckbox.addEventListener('change', async () => {
                    audioReactiveEnabled = audioReactiveCheckbox.checked;
                    if (audioReactiveEnabled) {
                        if (!isAudioSetup) {
                            ...
                            if (!audioReady) {
                                ...
                                showError("Failed to initialize audio. Reactivity disabled.", false);
                            }
                        } else {
                            requestAnimationFrame(processAudioFrame); // Ensure loop is running
                        }
                    } else {
